    ...
    with open(infile) as f:
        ...
        for line in lines:
            ...
            mapping[9] = [
                signal
                for signal in signals
                if len(signal) == 6 and signal | mapping[4] == signal
            ][0]
            signals.remove(mapping[9])
            mapping[3] = [
                signal
                for signal in signals
                if len(signal) == 5 and signal | mapping[1] == signal
            ][0]
            signals.remove(mapping[3])
            mapping[0] = [
                signal
                for signal in signals
                if len(signal) == 6 and signal | mapping[1] == signal
            ][0]
            signals.remove(mapping[0])
            mapping[6] = [signal for signal in signals if len(signal) == 6][0]
            signals.remove(mapping[6])
            mapping[5] = [
                signal for signal in signals if signal | mapping[6] == mapping[6]
            ][0]
